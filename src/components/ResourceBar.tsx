...
interface ResourceBarProps {
  ...
  onModelSwitch?: (model: 'local' | 'online') => void;
  onCalmModeToggle?: () => void;
  calmMode?: boolean;
  isExpanded?: boolean;
}

// RULE: Real OS metrics only, no fakes
// Update every ~1s, never animated for fun
// If something moves, something real is happening
export function ResourceBar({ metrics, onToggleExpanded, onModelSwitch, onCalmModeToggle, calmMode = false, isExpanded = false }: ResourceBarProps) {
  ...
  const getResourceBar = (value: number, maxValue: number = 100, color: string = 'blue') => {
    ...
    // REMOVE FLUFF: No fancy animations, just clean progress bars
    return (
      ...
      >
        <div className="px-4 py-2">
          <div className="flex items-center justify-between">
            ...
            {/* Center - Active model with switcher */}
              ...
              {metrics.activeModel && (
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center bg-slate-700 rounded-full p-0.5">
                    <button
                      onClick={() => onModelSwitch?.('local')}
                      className={`px-2 py-0.5 text-xs rounded-full transition-all ${
                        metrics.activeModel === 'local'
                    ...
                    <button
                      onClick={() => onModelSwitch?.('online')}
                      className={`px-2 py-0.5 text-xs rounded-full transition-all ${
                        metrics.activeModel === 'online'
                          ? 'bg-blue-600 text-white'
                          : 'text-gray-400 hover:text-white'
                      }`}
                    >
            ...
            <div className="flex items-center gap-2">
              ...
              {onCalmModeToggle && (
                <button
                  ...
                  title={calmMode ? 'Calm Mode ON - Reduced noise' : 'Enable Calm Mode'}
                >
                  Calm
                </button>
              )}
