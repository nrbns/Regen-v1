...
export function WorkflowAnalyticsDashboard() {
  ...
  const metrics = useMemo(() => {
    return templates.map(template => {
      ...
      return {
        ...
        successRate:
          completedJobs.length > 0 ? (completedJobs.length / templateJobs.length) * 100 : 0,
  ...
  const recommendations = useMemo(() => {
    ...
    const recentMetrics = metrics.filter(
      m => (m.lastUsed || 0) > recentThreshold && m.usageCount >= 3
    );
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <div className="mb-1 flex items-center gap-2 text-xs uppercase tracking-widest text-cyan-300">
          <BarChart3 size={14} />
          <span>Analytics</span>
        </div>
        ...
        <div className="rounded-lg border border-slate-700/50 bg-slate-800/20 p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-1 rounded bg-blue-500 px-3 py-1 text-xs font-medium text-white transition-colors hover:bg-blue-600"
            >
              <Zap size={12} />
      ...
      <div className="rounded-lg border border-slate-700/50 bg-slate-800/20 p-4">
        ...
        {metrics.length === 0 ? (
          <p className="text-xs text-slate-500">
            No workflow executions yet. Create and run workflows to see analytics.
          </p>
        ) : (
          <div className="space-y-2">
            {metrics
              ...
              .map((metric, idx) => (
                <motion.div
                  ...
                  className="cursor-pointer rounded-lg border border-slate-700/30 bg-slate-800/30 p-3 transition-colors hover:bg-slate-800/50"
  ...
  return (
    ...
    >
      <div className="flex gap-2">
        <div className="mt-0.5 flex-shrink-0">{icons[recommendation.type]}</div>
        <div className="min-w-0 flex-1">
          <div className="text-sm font-medium">{recommendation.title}</div>
          <p className="mt-1 text-xs opacity-80">{recommendation.description}</p>
          {recommendation.action && (
            <div className="mt-2 text-xs italic opacity-60">💡 {recommendation.action}</div>
          )}
