...
export function SystemBar() {
  ...
  const memoryMB = status?.memoryUsage
    ? (status.memoryUsage.heapUsed / 1024 / 1024).toFixed(0)
    : '--';
  const uptime = status ? Math.floor(status.uptime / 1000 / 60) + 'm' : '--';
  const redix = status?.redixAvailable ? 'ok' : 'unavailable';
  const batteryLevel =
    typeof status?.battery?.level === 'number' ? Math.round(status!.battery!.level * 100) : null;
  const charging = !!status?.battery?.charging;
  const cpu =
    typeof status?.cpuPercent === 'number'
      ? `${status.cpuPercent}%`
      : status?.cpu?.percent
        ? `${status.cpu.percent}%`
        : '--';
  ...
  return (
    <div className="ml-3 hidden items-center gap-3 text-xs text-gray-300 md:flex">
      <div className="flex items-center gap-1 rounded bg-[var(--surface-elevated)] px-2 py-1 text-[var(--text-muted)]">
        <Cpu className="h-4 w-4 text-gray-300" />
        <span className="font-medium">{memoryMB}MB</span>
        <div className="mr-2 flex items-center gap-2 text-[var(--text-muted)]">
