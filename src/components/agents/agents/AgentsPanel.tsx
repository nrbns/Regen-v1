...
export function AgentsPanel({
  ...
  // In v1-mode, render agents read-only and disable run actions
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredAgents.map(agent => (
              <AgentCard
                key={agent.id}
                {...agent}
                onRun={!minimalDemo ? () => handleRun(agent.id) : undefined}
                onPause={!minimalDemo && onPauseAgent ? () => onPauseAgent(agent.id) : undefined}
                onCancel={!minimalDemo && onCancelAgent ? () => onCancelAgent(agent.id) : undefined}
