...
export default function RamSavedCounter({ pollMs = 5000, variant = 'pill' }: RamSavedCounterProps) {
  ...
  if (variant === 'inline') {
    return (
      <div className="flex items-center gap-2 text-sm text-gray-100">
        ...
        <span className="font-semibold tabular-nums text-emerald-400">{todaySavedMb} MB</span>
        <button
  ...
  return (
    <div className="inline-flex items-center gap-1.5 rounded-md bg-gray-800/70 px-2.5 py-1 text-xs text-gray-200 shadow-sm">
      ...
      <span className="font-bold tabular-nums text-emerald-400">{todaySavedMb}</span>
      <span className="text-[10px] text-gray-400">MB</span>
      <button
        className="ml-1 rounded border border-gray-700 px-1.5 py-0.5 text-[10px] font-medium text-gray-300 transition-colors hover:bg-gray-700"
        onClick={handleShare}
        title="Copy to clipboard"
      >
        Share
