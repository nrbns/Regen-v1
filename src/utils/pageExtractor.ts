...
function extractMetaTags(document: Document): {
  ...
  const dateTag = document.querySelector(
    'meta[name="article:published_time"], meta[property="article:published_time"], time[datetime]'
  );
  if (dateTag) {
    meta.publishedDate =
      dateTag.getAttribute('datetime') || dateTag.getAttribute('content') || undefined;
  }
...
export function extractPageContent(document: Document, url?: string): PageMetadata {
  ...
  const mainContent = mainContentElement
    ? extractCleanText(mainContentElement)
    : extractCleanText(document.body || document);
