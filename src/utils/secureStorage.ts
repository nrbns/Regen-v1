 ...
 */
export async function storeSecure(key: string, value: string): Promise<boolean> {
  if (isTauriRuntime()) {
    try {
 ...
 */
async function storeSecureFallback(key: string, value: string): Promise<boolean> {
  try {
...
async function getSecureFallback(key: string): Promise<string | null> {
  try {
...
function deleteSecureFallback(key: string): boolean {
  try {
