...
export interface SummarizeOptions {
  ...
  onToken?: (token: string) => void; // CATEGORY C: Streaming token callback
  onProgress?: (progress: string) => void; // CATEGORY C: Progress updates
}
    ...
    if (response.status === 202) {
      ...
      return new Promise<SummarizeResult>((resolve, reject) => {
        const eventSource = new EventSource(
          `${subscribeUrl || `/api/ask?q=${encodeURIComponent(url || text || '')}`}`
        );
        let fullSummary = '';
