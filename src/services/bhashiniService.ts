...
const BHASHINI_API_KEY =
  import.meta.env.VITE_BHASHINI_API_KEY || (window as any).__BHASHINI_API_KEY;
...
const BHASHINI_LANGUAGE_MAP: Record<string, BhashiniLanguage> = {
  as: 'as',
  bn: 'bn',
  brx: 'brx',
  doi: 'doi',
...
export interface BhashiniTTSResponse {
  audioUrl: string;
  audioData?: ArrayBuffer; // If audio is returned directly
  format: 'mp3' | 'wav' | 'ogg';
}
 ...
 */
function normalizeLanguageCode(lang: string): BhashiniLanguage | null {
 ...
 */
export function isBhashiniSupported(language: string): boolean {
...
export async function translateWithBhashini(
...
): Promise<BhashiniTranslationResponse | null> {
  ...
  if (!sourceLang || !targetLang) {
    log.warn(
      `[Bhashini] Unsupported language pair: ${request.sourceLanguage} -> ${request.targetLanguage}`
    );
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      log.warn(
        `[Bhashini] Translation API request failed: ${response.status} ${response.statusText}`
      );
      return null;
    }
 ...
 */
export async function textToSpeechWithBhashini(
  request: BhashiniTTSRequest
): Promise<BhashiniTTSResponse | null> {
  if (!BHASHINI_API_KEY || BHASHINI_API_KEY === 'your_bhashini_api_key_here') {
 ...
 */
export async function speechToTextWithBhashini(
  request: BhashiniSTTRequest
): Promise<BhashiniSTTResponse | null> {
  if (!BHASHINI_API_KEY || BHASHINI_API_KEY === 'your_bhashini_api_key_here') {
 ...
 */
export function getBhashiniSupportedLanguages(): BhashiniLanguage[] {
