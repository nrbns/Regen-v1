import { createSnapshot, type SessionSnapshot } from '../core/recovery/snapshot';
...
async function restoreSnapshot(snapshot: SessionSnapshot) {
...
export async function exportSessionToFile(): Promise<void> {
  try {
...
export async function importSessionFromFile(file: File): Promise<void> {
  try {
    ...
    const payload = JSON.parse(text) as SessionExportPayload;
