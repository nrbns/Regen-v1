  ...
  if (!isIndexedDBAvailable()) {
    ...
    return {
      async getItem(key: string) {
        try {
          return localStorage.getItem(`${dbName}:${storeName}:${key}`);
        } catch {
          return null;
        }
      },
      async setItem(key: string, value: string) {
        try {
          localStorage.setItem(`${dbName}:${storeName}:${key}`, value);
        } catch {}
      },
      async removeItem(key: string) {
        try {
          localStorage.removeItem(`${dbName}:${storeName}:${key}`);
        } catch {}
      },
    };
  }
