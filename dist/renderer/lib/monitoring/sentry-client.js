...
function isValidSentryDSN(dsn) {
    ...
    if (dsn.includes('your-sentry-dsn') || dsn.includes('your_sentry_dsn'))
        return false;
...
async function shutdownRendererSentry() {
    if (!rendererSentry || !sentryInitialized)
        return;
    try {
...
export async function applyTelemetryOptIn(optIn) {
