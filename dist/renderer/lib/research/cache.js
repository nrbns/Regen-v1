 ...
 */
export async function cacheChunks(chunks) {
    try {
        const cachedChunks = chunks.map(chunk => ({
            ...chunk,
 ...
 */
export async function getCachedChunks(documentId) {
    try {
 ...
 */
export async function searchChunks(query, limit = 20) {
    try {
 ...
 */
export async function listCachedDocuments(limit) {
    try {
 ...
 */
export async function deleteCachedDocument(documentId) {
    try {
 ...
 */
export async function syncDocumentsFromBackend() {
    try {
        ...
        for (const doc of documents) {
            ...
            if (chunks.length > 0) {
                await cacheChunks(chunks.map((chunk, idx) => ({
