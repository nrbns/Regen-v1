...
export async function runAgent(task) {
    return traceFunction(`agent:${task.type}`, async () => {
...
async function handleQuickSummary(task) {
    ...
    const scraped = (await scrapeTool.execute({ url: task.url }));
...
async function handleDeepResearch(task) {
    ...
    if (!scrapeTool)
        throw new Error('scrapePage tool not found');
    const sources = await Promise.all(searchResults.results.slice(0, maxSources).map(async (result) => {
        try {
            const scraped = (await scrapeTool.execute({ url: result.url }));
...
async function handleCompareUrls(task) {
    ...
    if (!scrapeTool)
        throw new Error('scrapePage tool not found');
    const scraped = await Promise.all(task.urls.map(async (url) => {
        try {
