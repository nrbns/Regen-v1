    ...
    async execute(name, input, ctx) {
        ...
        try {
            ...
            if (error instanceof z.ZodError) {
                ...
                throw new Error(`Invalid input for tool ${name}: ${error.errors.map(e => e.message).join(', ')}`);
            }
            throw error;
        }
    }
...
toolRegistryV2.register({
    ...
    run: async (input, _ctx) => {
        ...
        if (!v1Tool)
            throw new Error('scrapePage tool not found');
...
toolRegistryV2.register({
    ...
    run: async (input, ctx) => {
        ...
        if (!v1Tool)
            throw new Error('summarizeText tool not found');
        const result = await v1Tool.execute(input);
        // Remember format preference
        ctx.memory.remember('preference', 'summary_format', input.format);
        return result;
    },
});
// Search web tool
toolRegistryV2.register({
    ...
    run: async (input, _ctx) => {
        ...
        if (!v1Tool)
            throw new Error('searchWeb tool not found');
        return v1Tool.execute(input);
    },
});
// Extract table tool
    ...
    run: async (input, _ctx) => {
        ...
        switch (typedInput.action) {
            case 'create':
                ...
                if (!typedInput.url)
                    throw new Error('URL required for create action');
            ...
            case 'close':
                ...
                if (!typedInput.tabId)
                    throw new Error('tabId required for close action');
                await ipc.tabs.close({ id: typedInput.tabId });
                return { success: true };
            case 'switch':
                requireCapability('navigate_tabs');
                if (!typedInput.tabId)
                    throw new Error('tabId required for switch action');
    ...
    run: async (input, _ctx) => {
        ...
        switch (typedInput.action) {
            case 'create':
                ...
                if (!typedInput.name)
                    throw new Error('name required for create action');
                const tabs = useTabsStore.getState().tabs;
            ...
            case 'restore':
                if (!typedInput.workspaceId)
                    throw new Error('workspaceId required for restore action');
