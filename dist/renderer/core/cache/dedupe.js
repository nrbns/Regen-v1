 ...
 */
export async function generateContentHash(url, content) {
 ...
 */
export async function isDuplicate(url, content) {
 ...
 */
export async function markAsScraped(url, content, result) {
    const hash = await generateContentHash(url, content);
    const cacheKey = `${DEDUPE_PREFIX}${hash}`;
 ...
 */
export async function getCachedScrape(url, content) {
    const hash = await generateContentHash(url, content);
    const cacheKey = `${DEDUPE_PREFIX}${hash}`;
    const cached = await cacheManager.get(cacheKey);
