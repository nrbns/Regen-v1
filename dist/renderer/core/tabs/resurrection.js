 ...
 */
export function saveTabForResurrection(tab, crashReason) {
    try {
 ...
 */
export function loadResurrectableTabs() {
    try {
 ...
 * Resurrect a tab
 */
export async function resurrectTab(resurrectable) {
    try {
        ...
        const newId = typeof result === 'object' && result && 'id' in result ? String(result.id) : String(result);
 ...
 */
export function deleteResurrectableTab(tabId) {
    try {
...
export function startAutoSaveTabs() {
    if (saveInterval) {
...
export function stopAutoSaveTabs() {
    if (saveInterval) {
        clearInterval(saveInterval);
        saveInterval = null;
    }
 ...
 */
export function checkForResurrectableTabs() {
    const all = loadResurrectableTabs();
    const now = Date.now();
 ...
 */
export function scheduleAutoResurrection(delayMs = RESURRECTION_DELAY_MS) {
    setTimeout(() => {
