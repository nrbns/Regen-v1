...
export const useModeManager = create((set, get) => ({
    currentMode: 'Browse',
    previousMode: null,
    modes: defaultModes,
    loading: false,
...
async function applyPrivacyDefaults(mode) {
...
export const ModeManager = {
    activate: (mode) => useModeManager.getState().activateMode(mode),
    register: (definition) => useModeManager.getState().registerMode(definition),
