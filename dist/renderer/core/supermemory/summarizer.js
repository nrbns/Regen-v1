 ...
 */
async function summarizeEventsWithAI(events) {
    try {
 ...
 */
function generateFallbackSummary(events) {
    const eventsByType = new Map();
    const urls = new Set();
 ...
 */
export async function compressEvents(events, summaryType = 'daily') {
    if (events.length === 0) {
...
async function saveSummary(summary) {
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            const summaryEvent = {
                id: summary.id,
                type: 'summary', // Special type for summaries
 ...
 * Get all summaries
 */
export async function getSummaries(limit) {
    try {
        const summaries = JSON.parse(localStorage.getItem('sm-summaries') || '[]');
 ...
 */
export async function runNightlySummarization() {
    try {
        const now = Date.now();
 ...
 */
export function initNightlySummarization() {
