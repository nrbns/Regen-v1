...
export class RedixPool {
     ...
     */
    returnSpan(span) {
        if (!span || this.spanPool.length >= this.maxPoolSize)
            return;
        span.innerHTML = '';
        span.className = '';
        if (span.parentNode) {
     ...
     */
    returnLink(link) {
        if (!link || this.linkPool.length >= this.maxPoolSize)
            return;
        link.href = '';
        link.innerHTML = '';
        link.className = '';
...
let redixPoolInstance = null;
/**
 * Get the global RedixPool instance
 */
export function getRedixPool() {
 ...
 */
export function resetRedixPool() {
    if (redixPoolInstance) {
