 ...
 */
function resolveSelector(selector, document) {
    try {
 ...
 */
function waitForElement(selector, document, timeout = 5000) {
 ...
 */
async function scrollIntoView(element, options = {}) {
    if (!(element instanceof HTMLElement))
        return;
 ...
 */
export async function readElement(selector, document = window.document) {
    const element = resolveSelector(selector, document);
    if (!element)
        return null;
    const rect = element.getBoundingClientRect();
 ...
 * Click on an element
 */
export async function clickElement(selector, options = {}, document = window.document) {
 ...
 * Fill an input field
 */
export async function fillInput(selector, value, options = {}, document = window.document) {
    const element = await waitForElement(selector, document, options.timeout);
    if (!element)
        return false;
 ...
 */
export async function readText(selector, document = window.document) {
    const element = resolveSelector(selector, document);
    if (!element)
        return null;
 ...
 */
export function readPageText(document = window.document) {
 ...
 */
export async function takeScreenshot(_options = {}, _document = window.document) {
 ...
 * Scroll the page
 */
export async function scrollPage(direction, amount, options = {}) {
 ...
 */
export async function waitForPageReady(document = window.document, timeout = 10000) {
    return new Promise((resolve) => {
 ...
 */
export function getPageInfo(document = window.document) {
    return {
 ...
 */
export async function waitForNavigation(currentUrl, timeout = 10000) {
    return new Promise((resolve) => {
        const startTime = Date.now();
        const check = () => {
 ...
 */
export async function extractStructuredData(document = window.document) {
    try {
 ...
 */
export async function saveToMemory(url, title, content, metadata) {
    try {
