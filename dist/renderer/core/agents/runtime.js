    ...
    registerDefaultAgents() {
        this.registerAgent({
            ...
            async entryPoint(input, env) {
                ...
                const memoryResults = memoryExecutor ? (await memoryExecutor({ query: input.prompt })) : [];
                steps.push({
            ...
            async entryPoint(input, env) {
                ...
                if (potentialSymbols.length > 0) {
                    try {
                        ...
                        if (tradeTool) {
                            const quotes = await Promise.allSettled(potentialSymbols.slice(0, 3).map((symbol) => tradeTool({ symbol })));
...
4. Market sentiment`;
    ...
    async persistAgentMemory(params) {
        try {
            useAgentMemoryStore
                .getState()
                .addEntry({
...
function buildContextPrompt(prompt, memoryResults) {
