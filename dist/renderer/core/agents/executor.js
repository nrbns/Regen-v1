 ...
 */
import { readElement, clickElement, fillInput, 
// readText, // Unused for now
readPageText, scrollPage, waitForPageReady, 
// getPageInfo, // Unused for now
extractStructuredData, saveToMemory, } from './primitives';
...
class AgentExecutor {
     ...
     */
    async requestConsent(action, context) {
    ...
    isDomainAllowed(url, context) {
        try {
            ...
            if (context.allowedDomains && context.allowedDomains.length > 0) {
                const isAllowed = context.allowedDomains.some((allowed) => hostname.includes(allowed.toLowerCase()));
                if (!isAllowed) {
                    return false;
                }
            }
     ...
     */
    async executeAction(action, context, stepIndex) {
...
export const executeActions = (actions, context) => executor.execute(actions, context);
export const getAuditLog = (runId) => executor.getAuditLog(runId);
export const cancelRun = (runId) => executor.cancel(runId);
