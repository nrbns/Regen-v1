 ...
 */
import { dispatchEvent as logEvent, getCurrentState, 
// getEventById, // Unused for now
// getEventCount, // Unused for now
getEventLog, getStateAtEventIndex, getStateAtTimestamp, initPersistence, redo as redoEventLog, undo as undoEventLog, } from './event-log';
...
function walkDiff(prev, next, path, depth, diffs) {
    if (depth > MAX_DIFF_DEPTH || diffs.length >= MAX_DIFF_ENTRIES) {
        return;
    }
    const keys = new Set([...Object.keys(prev || {}), ...Object.keys(next || {})]);
    keys.forEach((key) => {
        if (diffs.length >= MAX_DIFF_ENTRIES)
            return;
...
export const watch = (eventType, handler) => Redix.watch(eventType, handler);
export const dispatch = (event) => Redix.dispatch(event);
export const getRedixState = () => Redix.getState();
export const getRedixHistory = (eventType, limit) => Redix.getHistory(eventType, limit);
export const getRedixSnapshots = (limit) => Redix.getSnapshots(limit);
export const timeTravelRedix = (options) => Redix.timeTravel(options);
