...
class MemoryPool {
    ...
    async rebalance() {
        ...
        try {
            ...
            const reserveForActive = Math.min(totalBudget * 0.5, Math.max(this.options.reserveForActiveTabsMB, activeTabs.length * (this.options.tabCapMB / 2)));
    ...
    async setCap(tabId, capMB) {
        ...
        try {
            ...
            const record = { tabId, capMB, lastUpdated: Date.now() };
...
export function initializeMemoryPool() {
    memoryPool.start();
...
export function configureMemoryPool(options) {
