...
export function TabStrip() {
    ...
    const [holographicPreviewTabId, setHolographicPreviewTabId] = useState(null);
    const [hologramSupported, setHologramSupported] = useState(null);
    const [handoffStatus, setHandoffStatus] = useState({ platform: 'desktop', lastSentAt: null });
    ...
    const previousTabIdsRef = useRef(''); // Track tab IDs to prevent unnecessary updates
    const currentActiveIdRef = useRef(null); // Track active ID without causing re-renders
    const activationInFlightRef = useRef(null); // Track activations to avoid duplicate work
    const tabsRef = useRef(tabs); // Must be defined before refreshTabsFromMain and activateTab
    const fetchPredictiveSuggestionsRef = useRef(); // Must be defined before refreshTabsFromMain
    ...
    const handleCycleGroupColor = useCallback((group) => {
    ...
    const _handleDeleteGroup = useCallback((group) => {
        if (window.confirm(`Remove group "${group.name}"? Tabs will remain open.`)) {
            deleteGroup(group.id);
        }
    }, [deleteGroup]);
    const handleGroupDrop = useCallback((groupId) => {
    ...
    const handleGroupDragLeave = useCallback((groupId) => {
        setGroupDragTarget(current => (current === groupId ? null : current));
    }, []);
    const _handleCreateGroupShortcut = useCallback((name) => {
    ...
    const renderGroupHeader = useCallback((group) => {
        const groupTabs = filteredTabs.filter(t => t.groupId === group.id);
        return (_jsxs("div", { className: `flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-semibold ${groupDragTarget === group.id
                ? 'border-blue-500/70 bg-blue-500/10 text-blue-200'
                : 'border-gray-700/60 bg-gray-800/80 text-gray-200'}`, style: {
                borderLeft: `3px solid ${group.color}`,
            }, onDragOver: e => handleGroupDragOver(e, group.id), onDragLeave: () => handleGroupDragLeave(group.id), onDrop: e => {
                e.preventDefault();
                e.stopPropagation();
                handleGroupDrop(group.id);
                        ...
                        stopEventPropagation(e);
                    }, className: "p-1 rounded hover:bg-gray-700/60 transition-colors text-gray-300 hover:text-white", style: { zIndex: 10011, isolation: 'isolate' }, title: group.collapsed ? 'Expand group' : 'Collapse group', children: group.collapsed ? _jsx(ChevronRight, { size: 14 }) : _jsx(ChevronDown, { size: 14 }) }), _jsx("div", { className: "w-3 h-3 rounded-full border border-white/30 flex-shrink-0", style: { backgroundColor: group.color }, title: `Group color: ${group.color}` }), _jsx("button", { type: "button", onClick: e => {
                        stopEventPropagation(e);
                        handleRenameGroup(group);
                    }, onMouseDown: e => {
                        stopEventPropagation(e);
                    }, className: "text-[11px] font-semibold text-gray-100 hover:text-white truncate min-w-[60px]", style: { zIndex: 10011, isolation: 'isolate' }, title: "Rename group", children: group.name }), _jsxs("span", { className: "text-[10px] text-gray-400 ml-1", children: ["(", groupTabs.length, ")"] }), _jsxs("div", { className: "flex items-center gap-0.5 ml-auto", children: [_jsx("button", { type: "button", onClick: e => {
                                stopEventPropagation(e);
                                handleCycleGroupColor(group);
                            }, onMouseDown: e => {
                                stopEventPropagation(e);
                            }, className: "p-1 rounded hover:bg-gray-700/60 text-gray-400 hover:text-white transition-colors opacity-0 group-hover:opacity-100", style: { zIndex: 10011, isolation: 'isolate' }, title: "Change color", children: _jsx(Palette, { size: 12 }) }), _jsx("button", { type: "button", onClick: e => {
                                stopEventPropagation(e);
                                handleRenameGroup(group);
                            }, onMouseDown: e => {
                                stopEventPropagation(e);
                            }, className: "p-1 rounded hover:bg-gray-700/60 text-gray-400 hover:text-white transition-colors opacity-0 group-hover:opacity-100", style: { zIndex: 10011, isolation: 'isolate' }, title: "Rename group", children: _jsx(Edit3, { size: 12 }) })] })] }, `group-${group.id}`));
    ...
    const refreshTabsFromMain = useCallback(async () => {
        ...
        try {
            ...
            const mappedTabs = tabList.map((t) => {
    ...
    const renderTabNode = useCallback((tab) => {
        ...
        return (_jsx(TabHoverCard, { tabId: tab.id, children: _jsxs(motion.div, { id: `tab-${tab.id}`, "data-tab": tab.id, role: "tab", "aria-selected": tab.active, "aria-controls": `tabpanel-${tab.id}`, "aria-label": `Tab: ${tab.title || 'Untitled'}${tab.mode === 'ghost' ? ' (Ghost tab)' : tab.mode === 'private' ? ' (Private tab)' : ''}${tab.sleeping ? ' (Hibernating)' : ''}${group ? ` (Group: ${group.name})` : ''}`, tabIndex: tab.active ? 0 : -1, layout: true, initial: { opacity: 0, scale: 0.9, x: -20 }, animate: { opacity: 1, scale: 1, x: 0 }, exit: { opacity: 0, scale: 0.9, x: 20 }, transition: {
                    duration: 0.2,
                    ease: [0.4, 0, 0.2, 1],
                    layout: { duration: 0.3 }
                }, className: `
          ...
          `, style: {
                    ...
                    e.preventDefault();
                    window.__lastContextMenuPos = { x: e.clientX, y: e.clientY };
                    setContextMenu({
                        tabId: tab.id,
                        url: tab.url,
                    ...
                    if (holographicPreviewTabId === tab.id) {
                        setHolographicPreviewTabId(null);
                ...
                }, children: [prefetchForTab && (_jsx("span", { className: "absolute top-1 right-2 text-emerald-300 text-[11px] font-semibold", title: `Suggested follow-up: ${prefetchForTab.reason}`, children: "\u26A1" })), hologramSupported !== false && holographicPreviewTabId === tab.id && (_jsxs(motion.div, { initial: { opacity: 0, y: 6 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: 6 }, className: "absolute -bottom-9 left-1/2 flex -translate-x-1/2 items-center gap-2", children: [_jsxs(motion.button, { type: "button", className: "flex items-center gap-1 rounded-full border border-cyan-400/40 bg-cyan-500/15 px-3 py-1 text-[10px] text-cyan-100 shadow-lg backdrop-blur transition-colors hover:bg-cyan-500/25", onClick: e => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    setHandoffStatus({ platform: 'xr', lastSentAt: Date.now() });
                                    window.dispatchEvent(new CustomEvent('tab:holographic-preview', {
                                        detail: { tabId: tab.id },
                                    }));
                                    void ipc.crossReality.handoff(tab.id, 'xr');
                                }, children: [_jsx(Sparkles, { size: 12 }), " XR Hologram"] }), _jsxs(motion.button, { type: "button", className: "flex items-center gap-1 rounded-full border border-amber-400/40 bg-amber-500/15 px-2.5 py-1 text-[10px] text-amber-100 shadow-lg backdrop-blur transition-colors hover:bg-amber-500/25", onClick: e => {
                                    e.preventDefault();
                                    e.stopPropagation();
                            ...
                            stopEventPropagation(e);
                        }, "aria-label": tab.pinned ? `Unpin tab: ${tab.title}` : `Pin tab: ${tab.title}`, className: `${tab.pinned ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} p-0.5 rounded hover:bg-gray-700/50 transition-opacity text-gray-400 hover:text-blue-400 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 no-drag ml-1`, style: { pointerEvents: 'auto', zIndex: 10011, isolation: 'isolate' }, whileHover: { scale: 1.05 }, whileTap: { scale: 0.96 }, title: tab.pinned ? 'Unpin tab' : 'Pin tab', children: tab.pinned ? _jsx(Pin, { size: 14, className: "text-blue-400" }) : _jsx(PinOff, { size: 14 }) }), !tab.pinned && (_jsx(motion.button, { type: "button", onClick: e => {
                            e.preventDefault();
                            stopEventPropagation(e);
                            openPeek(tab);
                        }, onMouseDown: e => {
                            stopEventPropagation(e);
                        }, "aria-label": `Peek preview: ${tab.title}`, className: "opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-gray-700/50 transition-opacity text-gray-400 hover:text-gray-100 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 no-drag ml-1", style: { pointerEvents: 'auto', zIndex: 10011, isolation: 'isolate' }, whileHover: { scale: 1.05 }, whileTap: { scale: 0.96 }, title: "Peek preview", children: _jsx(Eye, { size: 14 }) })), !tab.pinned && (_jsx(motion.button, { type: "button", onClick: e => {
                            e.preventDefault();
                            stopEventPropagation(e);
                            closeTab(tab.id);
                        }, onAuxClick: e => {
                        ...
                        }, "aria-label": `Close tab: ${tab.title}`, className: "opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-gray-700/50 transition-opacity ml-1 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 no-drag", style: { pointerEvents: 'auto', zIndex: 2 }, whileHover: { scale: 1.1 }, whileTap: { scale: 0.9 }, title: "Close tab (Middle click)", children: _jsx(X, { size: 14, className: "text-gray-400" }) }))] }) }, tab.id));
    ...
    const predictiveRequestRef = useRef(null);
    ...
    const runPredictiveSuggestions = useCallback(async (options) => {
        if (!IS_ELECTRON) {
    ...
    useEffect(() => {
        if (!IS_ELECTRON) {
            ...
            const unsubscribe = ipcEvents.on('tabs:updated', (tabList) => {
                if (!isMounted)
                    return;
        ...
        const handleRestoring = (restoring) => {
            if (isMounted) {
                isRestoringRef.current = restoring;
            }
        };
        const unsubscribeRestoring = ipcEvents.on('session:restoring', handleRestoring);
        // Listen for restore tab messages (only once)
        const handleRestoreTab = async (tabState) => {
            if (!isMounted || isCreatingTabRef.current)
                return;
            isCreatingTabRef.current = true;
            try {
        ...
        const unsubscribeRestoreTab = ipcEvents.on('session:restore-tab', handleRestoreTab);
    ...
    const handlePrefetchOpen = useCallback((entry) => {
        if (!entry?.url)
            return;
    ...
    useEffect(() => {
        ...
        // Use requestAnimationFrame to ensure DOM is updated
        requestAnimationFrame(() => {
            const target = stripRef.current?.querySelector(`[data-tab="${CSS.escape(activeId)}"]`);
            if (target) {
    ...
    useEffect(() => {
        const handleModifiedShortcuts = (event) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { ref: stripRef, role: "tablist", "aria-label": "Browser tabs", className: "no-drag flex items-center gap-2 px-3 py-2 bg-[#1A1D28] border-b border-gray-700/30 overflow-x-auto scrollbar-hide relative", style: { pointerEvents: 'auto', position: 'relative', zIndex: 9999 }, onKeyDown: handleKeyNavigation, "data-onboarding": "tabstrip", onDragOver: e => {
                    ...
                    if (!stripRef.current?.contains(e.relatedTarget)) {
                        dragOverIndexRef.current = null;
                    }
                }, children: [_jsx(PredictivePrefetchHint, { entry: prefetchEntries[0] ?? null, onOpen: handlePrefetchOpen }), _jsx(PredictiveClusterChip, { clusters: predictedClusters, onApply: handleApplyCluster, summary: predictionSummary }), _jsxs("div", { className: "flex items-center gap-2 min-w-0 flex-1", style: { pointerEvents: 'auto' }, children: [_jsx(AnimatePresence, { mode: "popLayout", children: tabElements.length > 0 ? tabElements : null }), _jsxs("div", { className: "flex items-center gap-2 flex-shrink-0", children: [_jsxs(motion.button, { type: "button", onClick: e => {
                                            stopEventPropagation(e);
                  ...
                  `, title: recentlyClosed.length === 0
                                        ...
                                        }, "aria-label": "New tab", className: "p-2 rounded-lg hover:bg-gray-800/50 border border-transparent hover:border-gray-700/30 text-gray-400 hover:text-gray-200 transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2", style: { zIndex: 10011, isolation: 'isolate' }, whileHover: { scale: 1.05 }, whileTap: { scale: 0.95 }, title: "New Tab (Ctrl+T / \u2318T)", children: _jsx(Plus, { size: 18 }) })] })] }), contextMenu && (_jsx(Portal, { children: _jsx(TabContextMenu, { tabId: contextMenu.tabId, url: contextMenu.url, containerId: contextMenu.containerId ?? tabs.find(t => t.id === contextMenu.tabId)?.containerId, containerName: contextMenu.containerName ??
                                tabs.find(t => t.id === contextMenu.tabId)?.containerName, containerColor: contextMenu.containerColor ??
                                tabs.find(t => t.id === contextMenu.tabId)?.containerColor, mode: contextMenu.mode ?? tabs.find(t => t.id === contextMenu.tabId)?.mode, sleeping: contextMenu.sleeping ?? tabs.find(t => t.id === contextMenu.tabId)?.sleeping, onClose: () => setContextMenu(null) }) }))] }), _jsx(TabGroupsOverlay, { open: groupsOverlayOpen, onClose: () => setGroupsOverlayOpen(false) })] }));
}
