...
const resolveModeConfig = (config) => {
...
export function ModeSwitch() {
    ...
    const handleModeChange = (modeConfig) => {
    ...
    return (_jsxs("div", { className: "flex items-center gap-1 bg-gray-800/60 rounded-lg p-1 border border-gray-700/50 shadow-sm", children: [resolvedPrimary.map((m) => {
                const Icon = m.icon;
                const isActive = currentMode === m.id;
                const isDisabled = m.status === 'soon';
                const isBeta = m.status === 'beta';
                return (_jsxs(motion.button, { onClick: () => handleModeChange(m), whileHover: { scale: 1.02 }, whileTap: { scale: 0.98 }, className: `
            ...
            `, title: isDisabled ? `${m.label} (coming soon)` : m.label, disabled: isDisabled, children: [_jsx(Icon, { size: 14, className: isActive ? 'text-white' : m.color }), _jsx("span", { className: "hidden md:inline", children: m.label }), isBeta && !isDisabled && (_jsx("span", { className: "rounded-full bg-amber-500/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-amber-200", children: "Beta" })), isDisabled && (_jsx("span", { className: "hidden lg:inline text-[10px] uppercase tracking-wide text-slate-400/80", children: "Soon" })), isActive && (_jsx(motion.div, { className: `absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r ${m.glowColor} rounded-full`, initial: { scaleX: 0 }, animate: { scaleX: 1 }, transition: { type: 'spring', stiffness: 500, damping: 30 } }))] }, m.id));
            }), _jsxs(DropdownMenu, { children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsxs(motion.button, { whileHover: { scale: 1.02 }, whileTap: { scale: 0.98 }, className: `
              ...
              ${isSecondaryMode && currentModeConfig
                                ...
                                : 'text-gray-400 hover:text-gray-200 hover:bg-gray-700/30'}
            `, title: "More modes", children: [activeSecondaryMode ? (_jsxs(_Fragment, { children: [_jsx(activeSecondaryMode.icon, { size: 14, className: isSecondaryMode ? 'text-white' : activeSecondaryMode.color }), _jsx("span", { className: "hidden sm:inline", children: activeSecondaryMode.label })] })) : (_jsxs(_Fragment, { children: [_jsx(ChevronDown, { size: 14 }), _jsx("span", { className: "hidden sm:inline", children: "More" })] })), isSecondaryMode && currentModeConfig && (_jsx(motion.div, { className: `absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r ${currentModeConfig.glowColor} rounded-full`, initial: { scaleX: 0 }, animate: { scaleX: 1 }, transition: { type: 'spring', stiffness: 500, damping: 30 } }))] }) }), _jsxs(DropdownMenuContent, { align: "start", className: "w-48", children: [_jsx(DropdownMenuLabel, { className: "text-[10px] uppercase tracking-[0.2em] text-slate-400", children: "Additional Modes" }), _jsx(DropdownMenuSeparator, {}), resolvedSecondary.map((m) => {
                                const Icon = m.icon;
                                const isActive = currentMode === m.id;
                                const isDisabled = m.status === 'soon';
                                const isBeta = m.status === 'beta';
                                return (_jsxs(DropdownMenuItem, { onClick: () => handleModeChange(m), className: `
                  flex items-center gap-2 cursor-pointer
                  ${isActive ? 'bg-slate-800/60' : ''}
                  ${isDisabled ? 'opacity-60 cursor-not-allowed' : ''}
                `, disabled: isDisabled, children: [_jsx(Icon, { size: 14, className: isActive ? m.color : 'text-slate-400' }), _jsxs("div", { className: "flex flex-col", children: [_jsx("span", { className: isActive ? 'font-semibold' : '', children: m.label }), isBeta && !isDisabled && (_jsx("span", { className: "text-[10px] uppercase text-amber-300 tracking-wide", children: "Beta" })), isDisabled && (_jsx("span", { className: "text-[10px] uppercase text-slate-500 tracking-wide", children: "Coming soon" }))] })] }, m.id));
                            })] })] })] }));
}
