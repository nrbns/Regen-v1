...
export function BookmarkButton() {
    ...
    return (_jsx("button", { onClick: handleToggle, className: `p-1.5 rounded transition-colors ${isBookmarked ? 'text-yellow-400 hover:text-yellow-300' : 'text-gray-400 hover:text-gray-300'}`, title: isBookmarked ? 'Remove bookmark' : 'Add bookmark', "aria-label": isBookmarked ? 'Remove bookmark' : 'Add bookmark', children: _jsx(Star, { size: 16, className: isBookmarked ? 'fill-current' : '' }) }));
}
