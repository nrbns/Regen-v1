...
function deriveEffectiveMode(mode, battery) {
    if (mode !== 'Auto') {
        return mode;
    }
...
export const usePowerStore = create()(persist(set => ({
    battery: defaultBattery,
    selectedMode: 'Balanced',
    effectiveMode: 'Balanced',
    setBattery: status => set(state => {
        const nextBattery = {
            ...state.battery,
            ...status,
            supported: typeof status.supported === 'boolean' ? status.supported : state.battery.supported,
    ...
    onRehydrateStorage: () => state => {
        if (state) {
            state.effectiveMode = deriveEffectiveMode(state.selectedMode, state.battery ?? defaultBattery);
        }
    },
}));
