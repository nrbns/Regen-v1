...
export const useDownloadsStore = create()(persist((set, get) => ({
    downloads: [],
    ...
    updateDownload: (id, updates) => {
        set(state => ({
            downloads: state.downloads.map(d => d.id === id
                ? {
                    ...d,
                    ...updates,
    ...
    getActiveDownloads: () => {
        return get().downloads.filter(d => d.status === 'downloading' || d.status === 'queued' || d.status === 'paused');
    },
