...
export class RedixWS {
    ...
    errorLogged = false;
    isConnecting = false;
    backendOnline = isBackendAvailable();
    unsubscribeBackend;
    ...
    connect() {
        ...
        if (this.isConnecting || !this.backendOnline)
            return;
        ...
        try {
            ...
            this.ws.onclose = () => {
                ...
                if (import.meta.env.VITE_DISABLE_REDIX !== 'true' &&
                    import.meta.env.DISABLE_REDIX !== 'true') {
            ...
            if (!this.errorLogged && import.meta.env.DEV) {
                ...
                console.debug('[RedixWS] WebSocket connection unavailable - Redix is optional and will gracefully degrade.');
            }
        }
    }
    send(message) {
    ...
    startHttpStream(id, query, sessionId, options, onMessage) {
        ...
        const cleanup = () => {
            eventSource.removeEventListener('ack', handleAck);
    ...
    request(query, sessionId, options = {}, onMessage) {
...
export const getRedixWS = () => {
    if (!_redixWSInstance) {
        ...
        if (isWebMode ||
            ...
            _redixWSInstance.ws = null;
            _redixWSInstance.connect = () => { }; // No-op
            _redixWSInstance.isConnecting = false;
        }
        else {
            _redixWSInstance = new RedixWS();
        }
