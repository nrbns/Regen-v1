 ...
 */
export function isIndicLanguage(lang) {
    return INDIC_LANGUAGES.includes(lang);
}
/**
 ...
 */
async function detectLanguageIndicBERT(text) {
    try {
 ...
 */
async function detectLanguagemBART(text) {
    try {
        // Check if backend API is available
        const { ipc } = await import('../lib/ipc-typed');
 ...
 */
export async function detectLanguages(text, options) {
