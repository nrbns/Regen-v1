 ...
 */
export async function saveTabSnapshot(tabId, snapshot) {
    try {
 ...
 */
export async function loadTabSnapshot(tabId) {
    try {
        const database = await initDB();
 ...
 */
export function markTabActive(tabId) {
    const now = Date.now();
 ...
 */
function scheduleIdleCheck(tabId) {
...
async function suspendTab(tabId) {
    ...
    try {
        ...
        window.dispatchEvent(new CustomEvent('tab:suspended', {
            detail: { tabId },
        }));
    }
    catch (error) {
 ...
 */
export async function resumeTab(tabId) {
    ...
    if (snapshot) {
        ...
        window.dispatchEvent(new CustomEvent('tab:resumed', {
 ...
 */
export function initTabSuspension(tabId) {
    markTabActive(tabId);
}
/**
 ...
 */
export function setupBlurSuspension() {
    ...
    const handleBlur = () => {
        ...
        blurTimeout = setTimeout(async () => {
            if (!isBlurred)
                return; // Window might have regained focus
