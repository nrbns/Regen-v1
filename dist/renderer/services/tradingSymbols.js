...
export async function searchTradingSymbols(query) {
    ...
    const fallbackMatches = FALLBACK_RESULTS.filter(item => item.symbol.toLowerCase().startsWith(lower) || item.name.toLowerCase().includes(lower));
    cache.set(lower, { expiresAt: now + CACHE_TTL, results: fallbackMatches });
    return fallbackMatches;
}
function mapYahooQuoteToResult(quote) {
...
function inferInstrumentType(quote) {
    const quoteType = (quote.quoteType || '').toLowerCase();
    if (quoteType === 'cryptocurrency')
        return 'crypto';
    if (quoteType === 'etf')
        return 'fund';
    if (quoteType === 'forexpair')
        return 'forex';
    if (quoteType.includes('equity') || quoteType === 'stock')
        return 'stock';
    return 'other';
}
