...
export class ApiError extends Error {
    ...
    message;
    response;
    constructor(apiId, status, message, response) {
        super(`[${apiId}] ${message}`);
        this.apiId = apiId;
        this.status = status;
...
export class ExternalApiClient {
    ...
    async request(apiId, endpoint, options = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new ApiError(apiId, response.status, errorData.message || 'Request failed', errorData);
            }
     ...
     */
    async requestWithFallback(apiIds, endpoint, options = {}) {
        const errors = [];
        for (const apiId of apiIds) {
            try {
                const response = await this.request(apiId, endpoint, options);
