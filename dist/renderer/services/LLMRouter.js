...
async function isLocalLLMAvailable() {
    ...
    try {
        ...
        const result = await ipc.invoke('check_ollama_status');
 ...
 */
async function queryLocalLLM(request) {
    ...
    try {
        ...
        const response = await ipc.invoke('llm_query_local', {
 ...
 */
async function queryCloudLLM(request, provider = 'openai') {
    const startTime = performance.now();
    try {
 ...
 */
export async function queryLLM(request) {
    ...
    if (preferOffline) {
        ...
        if (localAvailable && isTauriRuntime()) {
            try {
                ...
                const unlisten = await listen('llm-stream-token', (event) => {
                    // Yield tokens as they arrive
                    return event.payload.text;
                });
                // Start streaming
                await ipc.invoke('llm_query_local_stream', {
                    prompt: request.prompt,
                    model: request.model || 'phi3:mini',
                });
                // Cleanup
