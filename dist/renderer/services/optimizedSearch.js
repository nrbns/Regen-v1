 ...
 */
export async function optimizedSearch(query, options) {
    ...
    if (preferProvider === 'auto' || preferProvider === 'brave' || preferProvider === 'multi') {
        searchPromises.push(multiSourceSearch(normalizedQuery, { limit: count, language })
            .then(results => results.map((r, idx) => ({
            title: r.title,
            url: r.url,
            snippet: r.snippet,
            domain: r.domain,
            provider: (r.source === 'brave' ? 'brave' : r.source === 'bing' ? 'bing' : 'multi'),
    ...
    if (preferProvider === 'auto' || preferProvider === 'duckduckgo') {
        searchPromises.push(fetchDuckDuckGoWeb(normalizedQuery, { count, language })
            .then(results => results.map((r, idx) => ({
            title: r.title,
            url: r.url,
            snippet: r.snippet,
    ...
    if (preferProvider === 'auto' || preferProvider === 'bing') {
        searchPromises.push(fetchBingSearch(normalizedQuery, { count, language })
            .then(results => {
    ...
    searchPromises.push(performLiveWebSearch(normalizedQuery, { count, language })
    ...
    searchPromises.push(fetchDuckDuckGoInstant(normalizedQuery, language)
        .then(result => {
        if (!result)
            return [];
    ...
    searchPromises.push(searchLocal(normalizedQuery)
        .then(results => results.map((r, idx) => ({
        title: r.title,
 ...
 */
function getFallbackResults(query, count) {
    ...
    try {
        ...
        if (hybridResults.length > 0) {
            return hybridResults.map((r, idx) => {
                ...
                if (r.source === 'brave')
                    provider = 'brave';
                else if (r.source === 'bing')
                    provider = 'bing';
                else if (r.source === 'duckduckgo')
                    provider = 'duckduckgo';
                return {
                    title: r.title,
                    url: r.url,
 ...
 */
export async function checkSearchProviders() {
