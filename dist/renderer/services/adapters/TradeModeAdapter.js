...
export class TradeModeAdapter {
    ...
    async getPrice(symbol) {
        ...
        if (coingecko) {
            try {
                const response = await this.client.request('coingecko', `/simple/price?ids=${symbol}&vs_currencies=usd&include_24hr_change=true`);
        ...
        if (alphaVantage) {
            try {
                const response = await this.client.request('alpha_vantage', `/query?function=GLOBAL_QUOTE&symbol=${symbol}&apikey=DEMO`);
        ...
        if (binance) {
            try {
                ...
                const response = await this.client.request('binance', `/ticker/24hr?symbol=${ticker}USDT`);
                return {
     ...
     */
    async getHistorical(symbol, timeframe = '1d') {
        const enabledApis = useExternalApisStore.getState().getEnabledApisForMode('trade');
                    ...
                    (timeframe === '1d'
                        ...
                        : timeframe === '1w'
                            ...
                            : timeframe === '1m'
                                ? 2592000
                                : 31536000);
                const response = await this.client.request('finnhub', `/stock/candle?symbol=${symbol}&resolution=${resolution}&from=${from}&to=${to}`);
                return {
                    symbol,
                    timeframe,
    ...
    async getMarketNews(symbol) {
        ...
        if (finnhub) {
            try {
                ...
                const response = await this.client.request('finnhub', endpoint);
 ...
 */
let tradeAdapterInstance = null;
export function getTradeAdapter() {
