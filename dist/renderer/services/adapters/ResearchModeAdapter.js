...
export class ResearchModeAdapter {
    ...
    async lookupDefinition(word) {
        ...
        if (dictApi) {
            try {
                const response = await this.client.request('dictionary_api', `/${word}`);
    ...
    async getSummary(topicOrUrl) {
        ...
        if (wikipedia && !isUrl) {
            try {
                ...
                const response = await this.client.request('wikipedia', `/page/summary/${pageTitle}`);
                return {
    ...
    async searchPapers(query) {
        ...
        if (openalex) {
            try {
                const response = await this.client.request('openalex', `/works?search=${encodeURIComponent(query)}&per_page=10`);
 ...
 */
let researchAdapterInstance = null;
