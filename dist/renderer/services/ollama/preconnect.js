 ...
 */
export async function preconnectOllama() {
    ...
    preconnectPromise = (async () => {
        try {
            ...
            const isAvailable = await invoke('check_ollama_status').catch(() => false);
            if (!isAvailable) {
 ...
 */
export function isOllamaPreconnected() {
