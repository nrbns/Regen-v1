 ...
 */
export async function parseIntent(command, snapshot) {
 ...
 */
function buildIntentPrompt(command, snapshot) {
 ...
 */
function parseLLMResponse(response, originalCommand) {
    ...
    try {
        ...
        return fallbackParse(originalCommand, { elements: [] });
    }
 ...
 */
function fallbackParse(command, snapshot) {
    ...
    if (lower.includes('click')) {
        ...
        if (buttonText) {
            const element = snapshot.elements.find(e => e.text?.toLowerCase().includes(buttonText.toLowerCase()) && e.interactive);
            if (element) {
                actions.push({
                    kind: 'click',
