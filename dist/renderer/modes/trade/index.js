...
import { getRealtimeMarketDataService } from '../../services/realtimeMarketData';
import { getTradeSignalService, } from '../../services/realtime/tradeSignalService';
...
export default function TradePanel() {
    ...
    useEffect(() => {
        const handleWisprTrade = (event) => {
            const { symbol, quantity, orderType, stopLoss } = event.detail;
            if (symbol) {
                // Find matching market
                const market = markets.find(m => m.name.toLowerCase().includes(symbol.toLowerCase()) || m.symbol.includes(symbol));
                if (market) {
                    setSelected(market);
                }
            }
            ...
            if (orderType && quantity) {
                toast.success(`${orderType === 'buy' ? 'Buy' : 'Sell'} order: ${quantity} ${symbol || selected.name}`);
            }
        };
    ...
    const { tick: _tick, connected: wsConnected, reconnecting: wsReconnecting, orderbook: wsOrderbook, } = useRealtimeTrade({
        ...
        onCandle: candle => {
            ...
            if (seriesRef.current) {
                ...
                try {
                    seriesRef.current.updateData?.(candleData);
                }
                catch {
    ...
    useEffect(() => {
        if (wsConnected)
            return; // Use WebSocket if connected
        const realtimeService = getRealtimeMarketDataService();
        let previousPrice = price;
        let high = price;
        let low = price;
        // Subscribe to real-time price updates
        const unsubscribe = realtimeService.subscribe(selected.symbol, (update) => {
        ...
        const initializeChart = () => {
            ...
            try {
                ...
                if (typeof addCandles !== 'function') {
                    console.error('[TradePanel] addCandlestickSeries is not available on chart instance', chart);
    ...
    useEffect(() => {
        if (wsOrderbook && wsConnected) {
            setOrderBookBids(wsOrderbook.bids
                .map((b) => ({
                price: b.price,
                quantity: b.size || b.quantity,
            }))
                .sort((a, b) => b.price - a.price));
            setOrderBookAsks(wsOrderbook.asks
                .map((a) => ({
                price: a.price,
    ...
    const handleOrderClick = useCallback((side) => {
    ...
    const handleConfirmOrder = useCallback(async () => {
        if (!pendingOrder || isPlacingOrder)
            return;
    ...
    return (_jsxs("div", { className: "flex h-full flex-col overflow-hidden bg-black text-white", children: [!wsConnected && lastPrice === null && (_jsx("div", { className: "p-4", children: _jsxs("div", { className: "text-center p-8 text-gray-400", children: [_jsx("p", { children: "Trade mode fallback - service unavailable" }), _jsx("p", { className: "text-sm mt-2", children: connectionError?.message || 'WebSocket disconnected' }), _jsx("button", { onClick: async () => {
                                setConnectionError(null);
                    ...
                    : 'border-yellow-500/40 bg-yellow-500/10 text-yellow-100'}`, children: _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("span", { children: [wsReconnecting ? '🔄 Reconnecting...' : '⚠️ Connection lost', lastPrice !== null && ' (Showing cached data)'] }), _jsx("button", { onClick: async () => {
                                ...
                                : 'bg-yellow-500/20 hover:bg-yellow-500/30'}`, children: wsReconnecting ? 'Cancel' : 'Retry' })] }) })), _jsxs("div", { className: "sticky top-0 z-30 flex flex-shrink-0 items-center justify-between border-b border-purple-800 bg-black/80 px-4 py-2 backdrop-blur", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(TrendingUp, { className: "h-5 w-5 text-emerald-400" }), _jsx("h1", { className: "text-sm font-semibold text-white md:text-lg", children: "Trade Mode" }), _jsx(SSEConnectionStatusIndicator, { showDetails: false }), _jsx("button", { onClick: () => setUseTradingView(!useTradingView), className: `ml-4 rounded-lg px-3 py-1 text-xs transition ${useTradingView
                                    ...
                                    ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/30'
                                    : 'text-gray-400 hover:bg-slate-700/50 hover:text-white'}`, title: "Switch to Research Mode", children: [_jsx(Sparkles, { size: 12 }), _jsx("span", { className: "hidden sm:inline", children: "Research" })] }), _jsxs("button", { onClick: () => setMode('Trade'), className: `flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium transition-all md:px-3 md:py-1.5 md:text-sm ${currentMode === 'Trade'
                                    ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/30'
                            ...
                            _jsxs("div", { ref: chartContainerRef, className: "relative h-[420px] w-full overflow-hidden rounded-lg border border-[#222] bg-[#030303]", children: [chartLoading && (_jsx("div", { className: "absolute inset-0 z-20 flex items-center justify-center bg-black/50", children: _jsxs("div", { className: "flex flex-col items-center gap-2", children: [_jsx(Loader2, { className: "h-8 w-8 animate-spin text-emerald-400" }), _jsx("span", { className: "text-sm text-gray-400", children: "Loading chart data..." })] }) })), _jsxs("div", { className: "absolute left-4 top-4 z-10 rounded-lg border border-gray-700 bg-black/70 px-4 py-2 text-xs backdrop-blur md:text-sm", children: ["1D \u2022 EMA \u2022 RSI \u2022 Volume Profile", wsConnected && _jsx("span", { className: "ml-2 text-green-400", children: "\u25CF Live" })] })] })), _jsx("div", { className: "mt-4", children: _jsx(TradesTape, { trades: recentTrades, symbol: selected.name }) })] }), _jsxs("div", { className: "col-span-12 flex flex-col gap-4 lg:col-span-4", children: [_jsx(OrderBook, { bids: orderBookBids, asks: orderBookAsks, onPriceClick: (price, _side) => {
                                    ...
                                    : isTablet
                                        ? 'py-4 text-2xl'
                                        : 'py-4 text-xl md:py-6 md:text-4xl'}`, title: "Buy (Press B)", children: isPlacingOrder ? (_jsxs(_Fragment, { children: [_jsx(Loader2, { className: "h-6 w-6 animate-spin" }), _jsx("span", { children: "PLACING..." })] })) : ('BUY') }), _jsx("button", { onClick: () => handleOrderClick('sell'), disabled: isPlacingOrder, className: `flex items-center justify-center gap-2 rounded-2xl bg-red-600 font-black shadow-2xl transition-all hover:bg-red-500 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 ${isMobile
                                    ? 'py-3 text-lg'
                                    : isTablet
                                        ? 'py-4 text-2xl'
                                        : 'py-4 text-xl md:py-6 md:text-4xl'}`, title: "Sell (Press S)", children: isPlacingOrder ? (_jsxs(_Fragment, { children: [_jsx(Loader2, { className: "h-6 w-6 animate-spin" }), _jsx("span", { children: "PLACING..." })] })) : ('SELL') })] }) }) }), _jsx(OrderConfirmModal, { isOpen: showConfirmModal, order: pendingOrder, onConfirm: handleConfirmOrder, onCancel: () => {
                    setShowConfirmModal(false);
                    setPendingOrder(null);
                }, warnings: pendingOrder && pendingOrder.quantity * pendingOrder.price > 100000
                    ? ['Large order size - ensure sufficient margin']
                    : [], errors: pendingOrder && pendingOrder.quantity <= 0 ? ['Quantity must be greater than 0'] : [] }), _jsx(TradeStagehandIntegration, {})] }));
}
