 ...
 */
export async function handleIframeBlocked(tabId, url) {
    ...
    if (isTauriRuntime()) {
        try {
            ...
            console.warn('[IframeBlockedFallback] Failed to open in main webview, trying external browser', error);
 ...
 */
export function setupIframeBlockedListener() {
    const handler = (event) => {
        handleIframeBlocked(event.detail.tabId, event.detail.url);
    };
    window.addEventListener('iframe-blocked', handler);
    return () => {
        window.removeEventListener('iframe-blocked', handler);
    };
}
