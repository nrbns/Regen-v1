...
export function validateCommandName(command) {
    ...
    if (dangerousCommands.some(dangerous => {
        ...
        if (wordBoundaryRegex.test(commandLower))
            return true;
        return false;
    })) {
 ...
 */
export function validateIpcRequest(command, payload) {
 ...
 */
export async function safeIpcInvoke(command, payload) {
    ...
    return ipc.invoke(command, safePayload);
}
