...
async function trySummarizeOnDevice(text, options) {
    try {
 ...
 */
export async function executeResearchAgent(request, reply) {
    const startTime = Date.now();
    const { query, maxResults = 5, language = 'en', useOnDeviceAI = false, includeCitations = true, format = 'report', } = request.body;
    ...
    try {
        ...
        const sourcesWithContent = await Promise.all(searchResults.slice(0, maxResults).map((result) => fetchLimiter(async () => {
            try {
                const contentCacheKey = `content:${result.url}`;
                let content = cache.content.get(contentCacheKey);
                if (!content) {
