 ...
 */
async function synthesizeContent(contents: SourceContent[], query: string): Promise<string> {
 ...
 */
export async function fuseContent(query: string, sourceUrls: string[]): Promise<FusedContent> {
  ...
  console.log(
    `[ContentFusion] Fused ${validContents.length} sources in ${Date.now() - startTime}ms`
  );
 ...
 */
export async function quickFuse(query: string, maxSources = 5): Promise<FusedContent> {
