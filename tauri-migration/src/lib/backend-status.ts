  ...
  if (!backendAvailable) {
    ...
    listeners.forEach(listener => {
      try {
        ...
        console.warn('[BackendStatus] Listener error (online):', error);
      }
    });
  }
}
...
export function markBackendUnavailable(reason?: unknown) {
  ...
  if (process.env.NODE_ENV !== 'production' && reason) {
    console.warn('[BackendStatus] Marking backend offline:', reason);
  }
  listeners.forEach(listener => {
    try {
      listener(false);
    } catch (error) {
      console.warn('[BackendStatus] Listener error (offline):', error);
    }
  });
}
