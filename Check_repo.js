...
checks.push({
  id: "rust_files",
  ok: exists("src-tauri/src/main.rs") || exists("src-tauri") && fs.readdirSync(path.join(root,"src-tauri")).some(f => f.endsWith(".rs")),
...
checks.push({
  id: "mock_llm",
  ok: exists("server/mock-llm.js") || exists("server/mock-llm") || exists("server/mock") || exists("server/mock-llm.py"),
...
if (pkg) {
  ...
  checks.find(c=>c.id==="build_scripts").note += ` Found scripts: ${Object.keys(scripts).slice(0,10).join(", ")}`;
}

// search keywords for AI/agent
const keywords = ["llm","ollama","huggingface","openai","agent","automate","automation","webview","browserview","tauri","whisper","tts","stt","gguf","ggml"];
const kwResults = searchFilesForKeywords(root, [".js",".ts",".jsx",".tsx",".rs",".py",".json",".md"], keywords);
...
const logKeywords = ["console.error","sentry","rollbar","winston","pino","logger","loggly","crash","error-report"];
...
// packaging
const packagingKeywords = ["tauri build","electron","pkg","nsis","deb","rpm","notarize","sign","codesign","tauri.conf.json"];
const pkgResults = searchFilesForKeywords(root, [".json",".md",".js",".sh",".rs"], packagingKeywords);
checks.push({
  id: "packaging",
...
checks.push({
  id: "ci_tests",
  ok: exists(".github") || exists(".gitlab-ci.yml") || exists("playwright.config.js") || exists("tests"),
...
if (kwFound>0) {
  Object.entries(kwResults).forEach(([k,files])=>{
    console.log(`- "${k}" found in ${Math.min(files.length,6)} file(s): ${files.slice(0,6).join(", ")}`);
  });
} else {
...
if (pkg) {
  console.log(`- package.json detected. name: ${pkg.name||"(unnamed)"} scripts: ${Object.keys(pkg.scripts||{}).slice(0,20).join(", ")}`);
} else {
...
if (pct >= 80) {
  ...
  console.log("⚠️ Your repo has a decent foundation but needs focused cleanup and a few missing modules.");
} else {
  console.log("⚠️ Your repo is missing several critical pieces. Don't panic — fixes are clear and doable.");
}

// Prioritized next steps based on missing checks
console.log("\nPRIORITIZED NEXT STEPS:");
const missing = checks.filter(c=>!c.ok).map(c=>c.id);
if (missing.includes("src_folder")) console.log("- Add or reorganize frontend source into /src (React/TS recommended).");
if (missing.includes("package_json")) console.log("- Create package.json and add dev/build scripts.");
if (missing.includes("tauri_folder") || missing.includes("rust_files")) console.log("- Add Tauri src-tauri or ensure Rust sources are present for desktop packaging.");
if (missing.includes("mock_llm")) console.log("- Add a small mock-llm (server/mock-llm.js) to simulate AI replies for development.");
if (missing.includes("build_scripts")) console.log("- Add npm scripts: dev, dev:web, dev:mock-llm, tauri:dev, build.");
if (missing.includes("ai_keywords")) console.log("- Integrate AI adapter module (llm client wrapper). Check for OpenAI/HuggingFace/Ollama usage.");
if (missing.includes("modular")) console.log("- Split code into modules: agent/, ai/, memory/, services/, components/.");
if (missing.includes("logging")) console.log("- Add basic logging and crash reporting (console + optional Sentry).");
if (missing.includes("packaging")) console.log("- Ensure tauri.conf.json or packaging scripts exist and test `tauri dev` locally.");
if (missing.includes("ci_tests")) console.log("- Add at least basic tests or a GitHub Actions workflow for builds.");
